...
proc hyperbus_constrain_pins { name clk_pin clk90_pin data_pins } {
    ...
    set_input_delay -add_delay -clock "${name}_clk" -max 1.00 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${name}_clk" -min -1.00 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${name}_clk" -clock_fall -max 1.00 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${name}_clk" -clock_fall -min -1.00 [get_ports "$data_pins"]

    set_output_delay -add_delay -clock "${name}_clk90" -max 1.00 [get_ports "$data_pins"]
    set_output_delay -add_delay -clock "${name}_clk90" -min -1.00 [get_ports "$data_pins"]
    set_output_delay -add_delay -clock "${name}_clk90" -clock_fall -max 1.00 [get_ports "$data_pins"]
    set_output_delay -add_delay -clock "${name}_clk90" -clock_fall -min -1.00 [get_ports "$data_pins"]
