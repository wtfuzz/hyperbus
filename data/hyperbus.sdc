...
proc hyperbus_constrain_pins { name clk clk90 data_pins } {
    ...
    set_input_delay -add_delay -clock "${clk}" -max 0.5 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${clk}" -min -0.5 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${clk}" -clock_fall -max 0.5 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${clk}" -clock_fall -min -0.5 [get_ports "$data_pins"]
