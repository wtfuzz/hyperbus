...
proc hyperbus_constrain_pins { name clk_pin clk90_pin data_pins } {
    ...
    set_input_delay -add_delay -clock "${name}_clk" -min -0.25 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${name}_clk" -clock_fall -max 1.00 [get_ports "$data_pins"]
    set_input_delay -add_delay -clock "${name}_clk" -clock_fall -min -0.25 [get_ports "$data_pins"]
